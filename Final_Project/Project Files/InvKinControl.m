function errors = InvKinControl(start, target)
...
%Establish the locations above the start and target
above_start = start;
...
[joint_pos, err] = invKinMove(cartesian_home, above_start, ur5, joint_pos, dx);
errors(1,:) = err;

% Step 2: Moving to the start config
pause(3);
disp("Step 2: Moving to start");
[joint_pos, err] = invKinMove(above_start, start, ur5, joint_pos, dx);
errors(2,:) = err;

% Step 3: Moving to above the start again
pause(3);
disp("Step 3: Moving back above start");
[joint_pos, err] = invKinMove(start, above_start, ur5, joint_pos, dx);
errors(3,:) = err;

% Step 4: Moving to above the target config
pause(3);
disp("Step 4: Moving above the target");
[joint_pos, err] = invKinMove(above_start, cartesian_home, ur5, joint_pos, dx);
errors(7,:) = err;
[joint_pos, err] = invKinMove(cartesian_home, above_target, ur5, joint_pos, dx);
errors(4,:) = err;

% Step 5: Moving to the target config
pause(3);
disp("Step 5: Moving to target");
[joint_pos, err] = invKinMove(above_target, target, ur5, joint_pos, dx);
errors(5,:) = err;

% Step 6: Moving back to above target
pause(3);
disp("Step 6: Moving back above target");
[~, err] = invKinMove(target, above_target, ur5, joint_pos, dx);
errors(6,:) = err;
end